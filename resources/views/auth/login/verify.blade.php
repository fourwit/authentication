@php
    ...
    $verificationChannel = $verificationChannel ?? ($pendingLoginOtp['channel'] ?? 'email');
    $verificationDestination = $verificationDestination ?? ($pendingLoginOtp['destination'] ?? ($pendingLoginOtp['email'] ?? $pendingLoginOtp['phone'] ?? 'your account'));
...
@if($useHostLayout && view()->exists($hostLayout))
    ...
    @section('content')
        <div class="max-w-md mx-auto py-10">
            <div class="text-center mb-8">
                <h1 class="text-3xl font-semibold tracking-tight">Verify your sign in</h1>
            ...
            @if (session('error'))
                <div class="mb-6 rounded-2xl bg-red-50 px-4 py-3 text-sm text-red-700 dark:bg-red-950/60 dark:text-red-300">
                    {{ session('error') }}
                </div>
            @endif

            <form id="code-form" method="POST" action="{{ route('authentication.login.verify.store') }}" class="space-y-6">
                @csrf
            ...
            <div class="mt-6 text-center">
                ...
                @endphp
                <form method="POST" action="{{ route('authentication.login.verify.resend') }}" class="inline" id="resend-form">
                    @csrf
            ...
            <div class="mt-6 text-center">
                <a class="text-sm text-slate-500 hover:text-slate-700 hover:underline dark:text-slate-400 dark:hover:text-slate-200" href="{{ route('login') }}">
                    Back to login
                </a>
            </div>
        </div>
    ...
    @endsection
@else
    <x-authentication::layouts.master title="Verify your sign in">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-semibold tracking-tight">Verify your sign in</h1>
            <p class="mt-2 text-sm text-slate-600 dark:text-slate-400">
                Enter the {{ $codeLength }}-digit code sent to your {{ $verificationLabel }} at <span class="font-medium text-slate-900 dark:text-slate-100">{{ $verificationDestination }}</span>.
